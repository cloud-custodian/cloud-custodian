    ...
    def process_resource_set(self, resource_set):
        ...
        for r in resource_set:
            ...
            if len(collected_metrics[key]) == 0:
                continue
            if self.data.get('percent_attr'):
                percent = (collected_metrics[key][0][self.statistics] /
                    r[self.data.get('percent_attr')]) * 100
                if self.op(percent, self.value):
                    matched.append(r)
            if self.op(collected_metrics[key][0][self.statistics], self.value) and not self.data.get('percent_attr'):
                matched.append(r)
        return matched
