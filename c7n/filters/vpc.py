...
class NetworkLocation(Filter):
    ...
    def process_resource(self, r, resource_sgs, resource_subnets, key):
        evaluation = []
        if 'subnet' in self.compare and len(resource_subnets):
            subnet_values = {
        ...
        if 'security-group' in self.compare and len(resource_sgs):
            sg_values = {
        ...
        if 'resource' in self.compare:
            ...
            if not self.missing_ok and r_value is None:
                evaluation.append({
                    ...
                    'resource': r_value})
            elif 'security-group' in self.compare and len(resource_sgs) and r_value not in sg_space:
                evaluation.append({
                    'reason': 'ResourceLocationMismatch',
                    'resource': r_value,
                    'security-groups': sg_values})
            elif 'subnet' in self.compare and len(resource_subnets) and r_value not in subnet_space:
                evaluation.append({
                    'reason': 'ResourceLocationMismatch',
                    'resource': r_value,
