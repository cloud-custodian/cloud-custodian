...
class Notify(EventAction):
    ...
    def send_sns(self, message):
        ...
        if topic.startswith('arn:aws:sns') == True:
            region = region = topic.split(':', 5)[3]
            topic_arn = topic
        else:
            region = self.manager.config.region
            owner_id = self.manager.config.account_id
            topic_arn = "arn:aws:sns:%s:%s:%s" % (region, owner_id, topic)
    ...
    def send_sqs(self, message):
        ...
        if queue.startswith('https://sqs') == True:
            region = queue.split('.', 2)[1]
            queue_url = queue
        elif queue.startswith('arn:aws:sqs') == True:
