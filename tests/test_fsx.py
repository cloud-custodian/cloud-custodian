...
class TestFSx(BaseTest):
    ...
    def test_fsx_delete_file_system_ontap_mock_exception(self):
        # Example of StorageVirtualMachineNotFound exception handling,
        # and example of VolumeNotFound exception handling.
        factory = self.replay_flight_data("test_fsx_delete_file_system_ontap")
        with patch("c7n.resources.fsx.local_session", autospec=True) as mock_local_session:
            mock_client = MagicMock()
            mock_local_session.return_value.client.return_value = mock_client
            ...
            mock_client.delete_storage_virtual_machine.side_effect = (
                mock_client.exceptions.StorageVirtualMachineNotFound(
                    {"Error": {"Code": "StorageVirtualMachineNotFound"}},
                    "DeleteStorageVirtualMachine"
                )
            )

            # Mock delete_volume to raise exception
            mock_client.delete_volume.side_effect = (
                mock_client.exceptions.VolumeNotFound(
                    {"Error": {"Code": "VolumeNotFound"}}, "DeleteVolume"
                )
            )
