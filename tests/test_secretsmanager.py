...
class TestSecretsManager(BaseTest):
    ...
    def test_secrets_manager_cross_account_deprecated(self):
        factory = self.replay_flight_data('test_secrets_manager_cross_account_deprecated')
        p = self.load_policy({
            'name': 'secrets-manager',
            'resource': 'secrets-manager',
            'filters': ['cross-account']},
    ...
    def test_mark_secret_for_op_deprecated(self):
        session = self.replay_flight_data("test_secrets_manager_mark_for_op_deprecated")
        client = session(region="us-east-1").client("secretsmanager")
        p = self.load_policy(
            {
                "name": "secrets-manager-resource",
