# Prerequisites for building the "provisionable" custom model test fixture.
#
...
# @terraform("bedrock_provisionable_custom_model") -- the model must outlive any
...
locals {
  ...
  # characters, so the warning and the role come first ("KEEP-c7n-provisio...").
  custom_model_name = "KEEP-c7n-provisionable-test-fixture"
  ...
  job_name_prefix = "c7n-jimfulton-provisionable-test-fixture"
  ...
  # Llama 3.1 8B Instruct is PT-supported but is NOT on the on-demand
  # custom-model-deployment eligible list, so provisioned throughput is its only
  # serving path -- exactly what the `provisioned-throughputs` filter's
  # populated case requires. See
  # https://docs.aws.amazon.com/bedrock/latest/userguide/prov-thru-supported.html
  base_model_identifier = "arn:aws:bedrock:us-west-2::foundation-model/meta.llama3-1-8b-instruct-v1:0:128k"
  ...
  fixture_tags = {
    KEEP    = "jimfulton provisionable test custom model"
...
resource "aws_s3_bucket" "training" {
  bucket        = "c7n-bedrock-provisionable-${random_pet.fixture.id}"
  force_destroy = true
}
...
resource "aws_s3_bucket" "output" {
  bucket        = "c7n-bedrock-provisionable-output-${random_pet.fixture.id}"
  force_destroy = true
}
...
resource "aws_iam_role" "customization" {
  name               = "c7n-bedrock-provisionable-${random_pet.fixture.id}"
