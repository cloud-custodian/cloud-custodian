...
class TestCleanRoomsMLConfiguredModelAlgorithm(BaseTest):

    def test_configured_model_algorithm(self):
        factory = self.replay_flight_data(
            "test_cleanroomsml_configured_model_algorithm")
        p = self.load_policy(
            {
                "name": "cleanroomsml-configured-model-algorithms",
                "resource": "aws.cleanroomsml-configured-model-algorithm",
            },
            session_factory=factory,
        )
        resources = p.run()
    ...
    def test_configured_model_algorithm_tag_untag(self):
        factory = self.replay_flight_data(
            "test_cleanroomsml_configured_model_algorithm_tag")
        client = factory().client("cleanroomsml")

        p = self.load_policy(
            {
                "name": "cleanroomsml-algorithm-tag",
                "resource": "aws.cleanroomsml-configured-model-algorithm",
                "filters": [
                    {"type": "value", "key": "name", "value": "c7n-authorized"}],
                "actions": [{"type": "tag", "key": "Env", "value": "Test"}],
            },
            session_factory=factory,
        )
        resources = p.run()
        self.assertEqual(len(resources), 1)
        arn = resources[0]["configuredModelAlgorithmArn"]
        tags = client.list_tags_for_resource(resourceArn=arn)["tags"]
        self.assertEqual(tags.get("Env"), "Test")

        p = self.load_policy(
            {
                "name": "cleanroomsml-algorithm-untag",
                "resource": "aws.cleanroomsml-configured-model-algorithm",
                "filters": [{"tag:Env": "Test"}],
                "actions": [{"type": "remove-tag", "tags": ["Env"]}],
            },
