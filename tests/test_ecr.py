...
class TestECR(BaseTest):
    ...
    def test_ecr_tags(self):
        ...
        self.assertEqual(
            tags,
            {'Env': 'Dev',
             'c7n_status': 'Resource does not meet policy: post-finding@2019/02/07'})

    def test_ecr_tags_deprecated(self):
        factory = self.replay_flight_data('test_ecr_tags_deprecated')
        p = self.load_policy({
            'name': 'ecr-tag',
            'resource': 'ecr',
            'filters': [{'tag:Role': 'Dev'}],
