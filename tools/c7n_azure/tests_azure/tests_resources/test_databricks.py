...
class DatabricksTest(BaseTest):
    def test_databricks_schema_validate(self):
        with self.sign_out_patch():
            p = self.load_policy(
                {'name': 'test-azure-databricks', 'resource': 'azure.databricks'}, validate=True
            )
            self.assertTrue(p)
...
class DatabricksVnetFilterTest(BaseTest):
    def test_databricks_vnet_filter_schema_prefix(self):
        ...
        self.assertEqual(len(resources), 1)
        self.assertEqual(
            resources[0]['c7n:Vnet']['properties']['addressSpace']['addressPrefixes'],
            ['10.0.0.0/16'],
        )
...
class DatabricksSubnetsFilterTest(BaseTest):
    def test_databricks_vnet_filter_schema_prefix(self):
        ...
        self.assertEqual(len(resources), 1)
        self.assertEqual(
            resources[0]['c7n:Subnets'][1]['properties']['addressPrefix'], '10.0.2.0/24'
        )
