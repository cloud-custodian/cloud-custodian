...
$ cd sam-deploy
$ aws cloudformation package --template-file deploy.yml --s3-bucket mybucket --output-template-file cfn.yml
...
def dispatch_render(p, sam):
    ...
    if properties['Role'].startswith('arn:') and "{account_id}" in properties['Role']:
        properties['Role'] = {'Fn::Sub': properties['Role'].replace("{account_id}", "${AWS::AccountId}")}
    elif properties['Role'].startswith('arn:'):
        pass
    else:
        properties['Role'] = {'Fn::Sub': "arn:aws:iam::${AWS::AccountId}:role/%s" % policy_lambda.role}
