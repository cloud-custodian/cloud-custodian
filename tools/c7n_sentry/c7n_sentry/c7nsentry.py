...
def process_log_event(event, context):
    ...
    if config['sentry_dsn']:
        ...
        sqs.send_message(QueueUrl=config['sentry_sqs'])
...
def process_log_group(config):
    ...
    factory = SessionFactory(config.region, config.profile, assume_role=config.role)
    session = factory()
    ...
    settings = dict(account_id=config.account_id, account_name=config.account_name)
...
def send_sentry_message(sentry_dsn, msg):
    ...
    auth_header = "Sentry %s" % ', '.join("%s=%s" % (k, v) for k, v in auth_header_keys)
    headers = {
        'User-Agent': client,
...
def get_sentry_message(config, data, log_client=None, is_lambda=True):
    ...
    try:
        _, level, logger, msg_frag = [
            s.strip() for s in error_msg[error_msg.find(',') :].split('-', 3)
        ]
    ...
    sentry_msg = {
        ...
        'tags': {'policy': policy, 'stream': stream, 'group': group},
    }
...
def get_function(
    ...
    config = dict(
        ...
        events=[CloudWatchLogSubscription(session_factory, log_groups, pattern)],
    )
...
def orgreplay(options):
    ...
    def process_account(a):
        ...
        if a['name'] not in projects:
            ...
            spost(
                endpoint + "teams/%s/%s/projects/" % (options.sentry_org, team_name),
                json={'name': a['name']},
            )
        ...
        for r in options.regions:
            ...
            b = bagger(region=r, log_group="/cloud-custodian/%s/%s" % (a['name'], r))

            try:
                process_log_group(b)
            except ClientError as e:
                log.warning("Could not process %s region %s error: %s", a['name'], r, e)
                ...
                for p in config.get('policies', ()):
                    ...
                    try:
                        ...
                        if e.response['Error']['Code']:
                            log.info(
                                "account: %s region: %s group: %s not found",
                                a['name'],
                                r,
                                b.log_group,
                            )
        ...
        for region_name in options.regions:
            for fname, config in account['config_files'].items():
                for policy in config.get('policies', ()):
                    if policy.get('mode'):
                        deploy_one(region_name, account, policy, options.sentry_dsn)
...
def deploy_one(region_name, account, policy, sentry_dsn):
    ...
    log.info("Deploying lambda for {} in {}".format(log_group_name, region_name))
...
def setup_parser():
    ...
    cmd_orgreplay.add_argument('--sentry-dsn', default=os.environ.get('SENTRY_DSN'))
    cmd_orgreplay.add_argument('--sentry-token', default=os.environ.get('SENTRY_TOKEN'))

    cmd_deploy = subs.add_parser('deploy')
    common_parser(cmd_deploy)
    cmd_deploy.add_argument('--sentry-dsn', default=os.environ.get('SENTRY_DSN'))
