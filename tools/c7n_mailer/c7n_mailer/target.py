...
class MessageTargetMixin(object):
    def handle_targets(
        self, message, sent_timestamp, email_delivery=True, sns_delivery=False
    ):
        ...
        if any(e.startswith('datadog') for e in message.get('action', ()).get('to')):
            ...
            try:
                datadog_delivery.deliver_datadog_messages(
                    datadog_message_packages, message
                )
            except Exception:
                traceback.print_exc()
                pass
        ...
        if any(
            e.startswith('splunkhec://') for e in message.get('action', ()).get('to')
        ):
