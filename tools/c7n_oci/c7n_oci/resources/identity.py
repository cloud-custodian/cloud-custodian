...
class UpdateCompartment(OCIBaseAction):
    ...
    """
    schema = type_schema('update_compartment',
                         params={'type': 'object'},
    ...
    def perform_action(self, resource):
        ...
        if self.data.get('params') and self.data.get('params').get('compartment_id'):
            ...
            params_dict['compartment_id'] = resource.get('identifier', additional_details.get('identifier'))
        if self.data.get('params').get('update_compartment_details'):
            update_compartment_details_user = self.data.get('params').get('update_compartment_details')
            params_model = self.update_params(resource, update_compartment_details_user)
            params_dict['update_compartment_details'] = oci.identity.models.UpdateCompartmentDetails(**params_model)
...
class UpdateGroup(OCIBaseAction):
    ...
    """
    schema = type_schema('update_group',
                         params={'type': 'object'},
                         rinherit=OCIBaseAction.schema)
    ...
    def perform_action(self, resource):
        ...
        if self.data.get('params') and self.data.get('params').get('group_id'):
            ...
            params_dict['group_id'] = resource.get('identifier', additional_details.get('identifier'))
...
class UpdateUser(OCIBaseAction):
    ...
    """
    schema = type_schema('update_user',
                         params={'type': 'object'},
                         rinherit=OCIBaseAction.schema)
    ...
    def perform_action(self, resource):
        ...
        if self.data.get('params') and self.data.get('params').get('user_id'):
            ...
            params_dict['user_id'] = resource.get('identifier', additional_details.get('identifier'))
             ...
             for resource in resources:
                 ...
                 if  'o_auth2_client_credentials' in resource:
                     ...
                     if o_auth2_client_credentials:
                         res['o_auth2_client_credentials'] = oci.util.to_dict(o_auth2_client_credentials)
                         result.append(res)
             return super().process(result)
        else:
             result = []
             for resource in resources:
                 ...
                 if o_auth2_client_credentials:
                     for o_auth2_client_credential in o_auth2_client_credentials:
                         res['o_auth2_client_credential'] = oci.util.to_dict(o_auth2_client_credential)
                         result.append(res)
