...
class UpdateCrossConnect(OCIBaseAction):
    ...
    """
    schema = type_schema('update_cross_connect',
                         params={'type': 'object'},
    ...
    def perform_action(self, resource):
        ...
        if self.data.get('params') and self.data.get('params').get('cross_connect_id'):
            ...
            params_dict['cross_connect_id'] = resource.get('identifier', additional_details.get('identifier'))
        if self.data.get('params').get('update_cross_connect_details'):
            update_cross_connect_details_user = self.data.get('params').get('update_cross_connect_details')
            params_model = self.update_params(resource, update_cross_connect_details_user)
            params_dict['update_cross_connect_details'] = oci.core.models.UpdateCrossConnectDetails(**params_model)
...
class UpdateVcn(OCIBaseAction):
    ...
    def perform_action(self, resource):
        ...
        if self.data.get('params') and self.data.get('params').get('vcn_id'):
            ...
            params_dict['vcn_id'] = resource.get('identifier', additional_details.get('identifier'))
...
class UpdateSubnet(OCIBaseAction):
    ...
    """
    schema = type_schema('update_subnet',
                         params={'type': 'object'},
                         rinherit=OCIBaseAction.schema)
    ...
    def perform_action(self, resource):
        ...
        if self.data.get('params') and self.data.get('params').get('subnet_id'):
            ...
            params_dict['subnet_id'] = resource.get('identifier', additional_details.get('identifier'))
