...
class TestZone(OciBaseTest):
    ...
    def test_add_defined_tag_to_zone(self, test, zone):
        ...
        policy = test.load_policy(
            {
                ...
                'actions': [
                    {
                        'type': 'update_zone',
                        'params': {'update_zone_details':{'defined_tags':self.get_defined_tag('add_tag')}}
                    }
                ]
            }
        ...
        policy.run()
        resources = self.get_resources(policy, compartment_id, id = zone_ocid, **self._get_zone_params(zone_name))
        ...
        policy = test.load_policy(
            {
                ...
                'actions': [
                    {
                        'type': 'update_zone',
                        'params': {'update_zone_details':{'defined_tags':self.get_defined_tag('update_tag')}}
                    }
                ]
            }
            ,session_factory=session_factory
        ...
        policy = test.load_policy(
            {
                ...
                'actions': [
                    {
                        'type': 'update_zone',
                        'params': {'update_zone_details':{'freeform_tags':{'Environment':'Development'}}}
                    }
                ]
            }
            ,session_factory=session_factory
        ...
        policy = test.load_policy(
            {
                ...
                'actions': [
                    {
                        'type': 'update_zone',
                        'params': {'update_zone_details':{'freeform_tags':{'Environment':'Production'}}}
                    }
                ]
            }
            ,session_factory=session_factory
