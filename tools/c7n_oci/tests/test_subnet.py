...
class TestSubnet(OciBaseTest):
    ...
    def test_add_defined_tag_to_subnet(self, test, subnet):
        ...
        '''
        compartment_id, subnet_ocid, subnet_name, vcn_id = self._get_subnet_details(subnet)
        ...
        policy = test.load_policy(
            {
                ...
                'actions': [
                    {
                        'type': 'update_subnet',
                        'params': {'update_subnet_details':{'defined_tags':self.get_defined_tag('add_tag')}}
                    }
                ]
            },
        ...
        policy.run()
        resources = self.get_resources(policy, compartment_id, id = subnet_ocid, **self._get_subnet_params(subnet_name, vcn_id))
        ...
        policy = test.load_policy(
            {
                ...
                'actions': [
                    {
                        'type': 'update_subnet',
                        'params': {'update_subnet_details':{'defined_tags':self.get_defined_tag('update_tag')}}
                    }
                ]
            },
            session_factory=session_factory
        ...
        policy = test.load_policy(
            {
                ...
                'actions': [
                    {
                        'type': 'update_subnet',
                        'params': {'update_subnet_details':{'freeform_tags':{'Environment':'Development'}}}
                    }
                ]
            },
            session_factory=session_factory
        ...
        policy = test.load_policy(
            {
                ...
                'actions': [
                    {
                        'type': 'update_subnet',
                        'params': {'update_subnet_details':{'freeform_tags':{'Environment':'Production'}}}
                    }
                ]
            },
            session_factory=session_factory
