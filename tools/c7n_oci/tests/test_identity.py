...
class TestIdentityTerraformTest(OciBaseTest):
    ...
    def _get_identity_compartment_details(self, identity_compartment):
        compartment_id = identity_compartment['oci_identity_compartment.test_compartment.compartment_id']
    ...
    def test_identity_compartment(self, identity_compartment, test):
        compartment_id, name = self._get_identity_compartment_details(identity_compartment)
        session_factory = test.oci_session_factory(self.__class__.__name__, inspect.currentframe().f_code.co_name)
        policy_str = {
            'name': 'filter-and-add-tags-on-compartments', 'description': 'Filter and add tags on the compartment',
            'resource': Resource.COMPARTMENT.value, 'filters': [{
                'type': 'query', 'params': {
                    'compartment_id': compartment_id
                }
            }, {
                'type': 'value', 'key': 'freeform_tags.Cloud_Custodian', 'value': 'True', 'op': 'eq'
            }], 'actions': [{
                'type': 'update_compartment', 'params': {
        ...
        policy.run()
        resources = self.get_resources(policy, compartment_id, **self._get_identity_compartment(name))
    ...
    def test_identity_group(self, identity_group, test):
        ...
        policy_str = {
            'name': 'filter-and-add-tags-on-group', 'description': 'Filter and add tags on the group',
            'resource': Resource.GROUP.value, 'filters': [{
                'type': 'query', 'params': {
                    'compartment_id': compartment_id
                }
            }, {
                'type': 'value', 'key': 'freeform_tags.Cloud_Custodian', 'value': 'Present', 'op': 'eq'
            }], 'actions': [{
                'type': 'update_group', 'params': {
    ...
    def test_identity_user_tag(self, identity_user, test):
        ...
        policy_str = {
            'name': 'filter-and-add-tags-on-user', 'description': 'Filter and add tags on the user',
            'resource': Resource.USER.value, 'filters': [{
                'type': 'query', 'params': {
                    'compartment_id': compartment_id
                }
            },
    ...
    def test_identity_user_cross_filter_size(self, identity_user, test):
        ...
        policy_str = {
            'name': 'filter_auth_tokens_based_on_size', 'description': 'Filter users with auth tokens equal to 2',
            'resource': Resource.USER.value, 'filters': [{
                'type': 'query', 'params': {
                    'compartment_id': compartment_id
                }
            }, {
                'type': 'auth_tokens', 'key': 'auth_tokens', 'value': '2', 'op': 'eq', 'value_type': 'size'
            }]
        }
        session_factory = test.oci_session_factory(self.__class__.__name__, inspect.currentframe().f_code.co_name)
        policy = test.load_policy(policy_str, session_factory=session_factory)
    ...
    def test_identity_user_cross_filter_age(self, identity_user, test):
        ...
        policy_str = {
            'name': 'filter_auth_tokens_based_on_age', 'description': 'Filter users with age less than 1',
            'resource': Resource.USER.value, 'filters': [{
                'type': 'query', 'params': {
                    'compartment_id': compartment_id
                }
            }, {
                'type': 'auth_tokens', 'key': 'auth_token.time_created', 'value': '2023/01/01', 'op': 'greater-than',
                'value_type': 'date'
            }]
        }
        session_factory = test.oci_session_factory(self.__class__.__name__, inspect.currentframe().f_code.co_name)
...
class IdentityUnitTest(unittest.TestCase, OciBaseTest):
    ...
    def get_action(resource):
        ...
        return [{
            ...
            'params': {
                method_param: {
                    'freeform_tags': {
                        'Environment': 'Cloud-Custodian-Dev'
                    }
                }
            }
        }]
    ...
    def test_identity_compartment_schema(self):
        self.assertTrue(
            self.load_policy(self.get_policy('compartment', [self.get_query_filter()], self.get_action('compartment')),
                             validate=True))

    def test_identity_group_schema(self):
        self.assertTrue(self.load_policy(self.get_policy('group', [self.get_query_filter()], self.get_action('group')),
                                         validate=True))

    def test_identity_user_schema(self):
        self.assertTrue(self.load_policy(self.get_policy('user', [self.get_query_filter()], self.get_action('user')),
                                         validate=True))

    def test_identity_api_key_schema(self):
        self.assertTrue(self.load_policy(
            self.get_policy('user', [self.get_query_filter(), self.get_cross_resource_filter('api_key')]),
            validate=True))

    def test_identity_auth_token_schema(self):
        self.assertTrue(self.load_policy(
            self.get_policy('user', [self.get_query_filter(), self.get_cross_resource_filter('auth_token')]),
            validate=True))

    def test_identity_db_credential_schema(self):
        self.assertTrue(self.load_policy(
            self.get_policy('user', [self.get_query_filter(), self.get_cross_resource_filter('db_credential')]),
            validate=True))

    def test_identity_customer_secret_key_schema(self):
        self.assertTrue(self.load_policy(
            self.get_policy('user', [self.get_query_filter(), self.get_cross_resource_filter('customer_secret_key')]),
            validate=True))

    def test_identity_smtp_credential_schema(self):
        self.assertTrue(self.load_policy(
            self.get_policy('user', [self.get_query_filter(), self.get_cross_resource_filter('smtp_credential')]),
            validate=True))

    def test_identity_oauth_credential_schema(self):
        self.assertTrue(self.load_policy(self.get_policy('user', [self.get_query_filter(),
                                                                  self.get_cross_resource_filter(
                                                                      'o_auth2_client_credential')]), validate=True))
